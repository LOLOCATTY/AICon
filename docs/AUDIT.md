...
> Not yet done: **F04/F05** (migrating `run_code` off the legacy CodeDom compiler onto Roslyn) — held
> back deliberately as a separate, larger, riskier change to the AI's most-used tool, worth its own
> review rather than bundling into this pass. **F13** (code signing) and **F17** (test project) are
> process/infrastructure items, not code changes, and are unchanged. **F01** and **F10** were
> informational only and needed no code change. See each row below for what actually happened.
...
2. **F03 — Unauthenticated localhost bridge + unsandboxed `run_code`.** ✅ Bridge auth fixed; `run_code`
   itself is still unsandboxed by design (see F04, deferred). Requires local code execution
...
8. **P2 — Fix F04/F05.** Migrate `run_code` onto `AiconScriptCompiler`; delete the `CSharpCodeProvider`
   path and the `err.Line - 13` remap entirely.

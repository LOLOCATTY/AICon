...
Same 57 Revit tools as the Claude/MCP build — one shared `ToolRegistry` feeds both, so the two
paths never drift.
...
| `..\shared\ToolRegistry.cs` | the 57 tools — shared with the MCP server |
